...
###############
Matrix_Q = function(comp_mat,K,datA,datB){
 ...
 # identification = apply(matrix_id,1, FUN= function(x){which(x==1)}) # common variables
  ...
  dif=which(naive_id!=idBA)
  
  return(list(Q=Q, naive_id=naive_id,dif=dif,number=number))
